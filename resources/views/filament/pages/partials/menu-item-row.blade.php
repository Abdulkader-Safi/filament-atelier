{{--
    ...
    only ever build one level deep). Kept deliberately sparse after the
    first cut: label only, no URL in the row, edit opens a Filament action
    modal rather than expanding the row in place. Matches Shopify's own
    menu-item list, checked on Mobbin: a drag handle, the label, "Edit" and
    "Delete", nothing else.
--}}
<div
    wire:key="menu-row-{{ $item['id'] }}"
    @class([
        'group flex items-center gap-1 border-t border-gray-100 py-1.5 pe-2 first:border-t-0 dark:border-white/5',
        'bg-gray-50/60 ps-8 dark:bg-white/[0.02]' => $isChild,
        'ps-1' => ! $isChild,
    ])
>
    <x-filament::icon icon="heroicon-m-bars-2" class="h-4 w-4 shrink-0 text-gray-300 dark:text-gray-600" />
...
@unless ($isChild)
    ...
    @if ($this->canNest())
        <div class="border-t border-gray-100 bg-gray-50/60 py-1.5 ps-8 dark:border-white/5 dark:bg-white/[0.02]">
