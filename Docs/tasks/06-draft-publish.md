...
- [ ] All editing writes `draft_content`.
- [ ] Publish copies draft to published, sets `status` and `published_at`.
- [ ] Public route reads `published_content` only, never the draft.
- [ ] Unpublished page returns 404 on the public route.
- [ ] Status badge, including the "published with unpublished changes" state (compare the two columns).
- [ ] Unpublish, returning the page to draft without losing content.

### Revisions

- [ ] Snapshot the full tree into `page_revisions` on every publish.
- [ ] Record who published and when.
- [ ] Keep the last N, configurable, with a default that won't grow the table forever.
...
- [ ] `temporarySignedRoute` to the preview controller for the shareable link.
- [ ] Configurable expiry.
- [ ] Optional rotatable `preview_token` for a stable link, and a way to rotate it.
- [ ] `noindex` and no analytics on every preview response.
- [ ] The link renders the draft in a chosen locale.

### Housekeeping

- [ ] Regenerate the sitemap on publish and on unpublish (feature 07).
- [ ] Clear any cached page output on publish (feature 09).
